                ...
                foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
                    {
                        if ($delivery_id !== 0 && intval($delivery_id) <= 0)
                        {
                            ...
                            foreach ($arDelivery["PROFILES"] as $profile_id => $arProfile)
                            {
                                ...
                                if (is_array($arDelivery["LOGOTIP"]) && count($arDelivery["LOGOTIP"]) > 0):
                        ...
                        {
                            if (is_array($arDelivery["STORE"]) && count($arDelivery["STORE"]) > 0)
                                $clickHandler = "onClick = \"submitForm();fShowStore('".$arDelivery["ID"]."','".$arParams["SHOW_STORES_IMAGES"]."','".$width."','".SITE_ID."'); BX('ID_DELIVERY_ID_".$arDelivery["ID"]."').checked=true;\"";
                            else
                            ...
                            <div class="form-check form-check-inline disabled">
                                ...
                                <label class="form-check-label" for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" <?=$clickHandler?>>
                                    <input type="radio"
                                           ...
                                           <?(!count(is_array($arDelivery["STORE"]) ? $arDelivery["STORE"]: [] )) ? 'onclick="submitForm();"' : ''?>
                                    />
