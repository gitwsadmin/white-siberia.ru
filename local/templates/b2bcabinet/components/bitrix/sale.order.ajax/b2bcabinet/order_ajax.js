...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.userProfileBlockNode = BX(parameters.userProfileBlockId);
		...
		{
			var userProfiles = this.result.USER_PROFILES;
			var form;
			...
			{
				// this.result.USER_PROFILES = userProfiles;
				var data = this.getData(action, actionData);
				if (data.order.PROFILE_ID) {
					data.order.PERSON_TYPE = userProfiles[data.order.PROFILE_ID].PERSON_TYPE_ID;
				}
				...
				BX.ajax({
					...
					onsuccess: BX.delegate(function(result) {
						result.order.USER_PROFILES = this.result.USER_PROFILES;
		...
		animateScrollTo: function(node, duration, shiftToTop)
		{
			...
			$('.content-scroll').animate({ scrollTop: orderBlockPos.height + ghostTop - orderBlockPos.bottom }, 200);
		},
		...
		{
			const allSections = this.orderBlockNode.querySelectorAll('.bx-soa-section');
			allSections.forEach((item) => {
				if(!this.result.SHOW_AUTH && item.id == this.authBlockNode.id) return;
				// if(item.id == this.propsBlockNode.id) return;
				this.show(item);
			})
		},
		...
		{
			this.getProfilesControl(this.userProfileBlockNode);
		},
		/**
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			var mainColumns = [],
				...
				i, tr, cols, itemImgNode, itemImgBlock, itemImg;
			...
			itemImgNode = BX.create('DIV', {props: {className: 'bx-soa-item-td bx-soa-item-td--img'}});
			itemImgBlock = BX.create('DIV', {props: {className: 'bx-soa-item-block-img'}});

			if (item.data.PREVIEW_PICTURE_SRC && item.data.PREVIEW_PICTURE_SRC.length)
				itemImg = this.getImageSources(item.data, 'PREVIEW_PICTURE');
			else if (item.data.DETAIL_PICTURE_SRC && item.data.DETAIL_PICTURE_SRC.length)
				itemImg = this.getImageSources(item.data, 'DETAIL_PICTURE');

			if (itemImg && itemImg.src_2x)
			{
				itemImgBlock.setAttribute('style',
					'background-image: url(' + itemImg.src_1x + ');' +
					'background-image: -webkit-image-set(url(' + itemImg.src_1x + ') 1x, url(' + itemImg.src_2x + ') 2x)'
				);
			}
			else
			{
				itemImg = itemImg && itemImg.src_1x || this.defaultBasketItemLogo;
				itemImgBlock.setAttribute('style', 'background-image: url(' + itemImg + ');');
			}

			itemImgNode.appendChild(itemImgBlock);
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'col-md-6 form-group bx-soa-location-input-container'},
			...
			if (location && location[0])
			{
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-reference col-12'},
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				propsItemNode = BX.create('DIV', {props: {className: "col-md-6 form-group bx-soa-location-input-container"}});
				...
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						if (currentType.CHECKED == 'Y') {
							node.appendChild(BX.create('input', {
								props: {type: 'hidden', name: 'PERSON_TYPE', value: currentType.ID}
							}));
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'row bx-soa-pp-item-container'}}),
				paySystemItemNode, i;
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				title, label, itemNode, tooltipPay;
			...
			if (item.DESCRIPTION) {
				tooltipPay = BX.create('DIV', {
					props: {className: 'tooltip-info'},
					children: [
						BX.create('I', {
							props: {
								className: 'icon-question7'
							}
						})
					]
				});
				tooltipPay.setAttribute('data-popup', 'tooltip');
				tooltipPay.setAttribute('data-original-title', item.DESCRIPTION);
			}
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'row bx-soa-pp-item-container'}}),
				deliveryItemNode, k;
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, tooltip;
			...
			if (item.DESCRIPTION) {
				...
				tooltip.setAttribute('data-popup', 'tooltip');
				tooltip.setAttribute('data-original-title', item.DESCRIPTION);

				labelNodes.push(tooltip);
			}
		...
		editPropsItems: function(propsNode)
		{
			...
			while (group = groupIterator())
			{
				...
				let nameGroupNode = BX.create('H4', {
					props: {className: 'bx-soa-section-title-container'},
					text: group.getName()
				});

				propsItemsContainer = BX.create('DIV', {
					props: {className: 'col-md-6 bx-soa-customer'},
					children: [nameGroupNode]
				});
		...
		editTotalBlock: function()
		{
			...
			this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY') + total.BASKET_POSITIONS, priceHtml, params));
						...
						children: [
							BX.create('A', {
								props: {
									...
									className: 'btn btn_b2b btn-lg btn-order-save'
								},
		...
		editMobileTotalBlock: function()
		{
			...
			// this.mobileTotalBlockNode.appendChild(document.querySelector('.bx-soa-cart-total-title').cloneNode(true));
