...
<?if($arResult['ITEMS']):?>
	...
	<div class="maxwidth-theme only-on-front mwnew">
		<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL'] || ($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW'])):?>
					...
					<div class="row">
						<div class="col-md-3">
							<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL']):?>
								<?=CMax::showIconSvg("forh1", SITE_TEMPLATE_PATH.'/images/svg/forh1.svg');?><h3><?=CMax::showIconSvg("forh1", SITE_TEMPLATE_PATH.'/images/svg/forh1.svg');?><?=$arParams['TITLE_BLOCK'];?></h3>
								<?// intro text?>
								<?if($arParams['INCLUDE_FILE']):?>
								...
								<div class="block-links">									
									...
									<?if($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW']):?>
										<span><span class="btn btn-transparent-border-color btn-sm animate-load" data-event="jqm" data-param-form_id="REVIEW" data-name="send_review" title="<?=$arParams['TITLE_ADD_REVIEW'] ;?>"><?=CMax::showIconSvg("resume colored", "/images/nb/ost.svg", "", "", true, false);?></span></span>
									<?endif;?>
								</div>
							<?endif;?>
						</div>
			...
			<?else:?>
				<div class="top_block">
					<h3><?=CMax::showIconSvg("forh1", SITE_TEMPLATE_PATH."/images/svg/forh1.svg");?><?=$arParams['TITLE_BLOCK'];?></h3>
		...
		?>
		<div class="item-views nr2 reviews<?=($bCompact ? ' compact' : '');?><?=($bMoreItem ? ' more-item' : '');?> <?=($arParams['LINKED_MODE'] == 'Y' ? ' linked ' : '');?> <?=($notSlider ? ' list-mode ' : '');?>">
			<?if($notSlider):?>
				<div class="appear-block loading_state<?=(!$bOneItem ? ' shadow' : '');?>" >
			<?else:?>
				<div class="owl-carousel owl-theme owl-bg-nav short-nav hidden-dots visible-nav swipeignore wsmooth1 appear-block loading_state<?=(!$bOneItem ? ' shadow' : '');?>" data-plugin-options='{"nav": true, "dots": false, "items":1, "loop": false, "marginMove": false, "autoplay": false, <?=($animationSpeed >= 0 ? '"smartSpeed": '.$animationSpeed.',' : '')?> "useCSS": true, "responsive": {"0":{"items": 1, "autoWidth": true, "lightDrag": true, "margin":-1}, "601":{"items": 1, "autoWidth": false, "lightDrag": false, "margin":0}, "1200":{"items": <?=$col?>, "margin":1}}}'>
			<?endif;?>
					...
					<div class="item-wrapper col-xs-12<?=(!$bOneItem ?  ' bg-fill-white bordered1 box-shadow1' : '');?>">
						<div class="item clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered box-shadow' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<div class="newItemRev">
									...
									<?if(isset($arItem['FIELDS']['PREVIEW_TEXT']) && $arItem['FIELDS']['PREVIEW_TEXT']):?>
										<div class="body-info">
											...
											<div class="preview-text">
												<? if($arItem['DISPLAY_PROPERTIES']['DOST']['VALUE'] || $arItem['DISPLAY_PROPERTIES']['NEDOST']['VALUE'] || $arItem['DISPLAY_PROPERTIES']['COMMENT']['VALUE']){ ?>
													<div class="toab1">
														<? if($arItem['DISPLAY_PROPERTIES']['DOST']['VALUE']){ ?>
															<div class="ab1"><b>Достоинства: </b><?=$arItem['DISPLAY_PROPERTIES']['DOST']['VALUE']?></div>
														<? } ?>
														<? if($arItem['DISPLAY_PROPERTIES']['NEDOST']['VALUE']){ ?>
															<div class="ab1"><b>Недостатки: </b><?=$arItem['DISPLAY_PROPERTIES']['NEDOST']['VALUE']?></div>
														<? } ?>
														<? if($arItem['DISPLAY_PROPERTIES']['COMMENT']['VALUE']){ ?>
															<div class="ab1"><b>Комментарий: </b><?=$arItem['DISPLAY_PROPERTIES']['COMMENT']['VALUE']?></div>
														<? } ?>
													</div>
												<? } ?>
											...
											<?if(($arParams['PREVIEW_TRUNCATE_LEN'] > 0) && (strlen($arItem['~PREVIEW_TEXT']) > $arParams['PREVIEW_TRUNCATE_LEN'])):?>
												<div class="link-block-more">
													<span><span class="btnmore animate-load" data-event="jqm" data-param-id="<?=$arItem['ID'];?>" data-param-type="review" data-name="review">Показать полностью...</span></span>
												</div>
											<?endif;?>
											...
											<?endif;?>
											<?if($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'] && false):?>
								...
								<? if(is_array($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE']) && count($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'])>0){?>
									<?
										$FILE1 = CFile::GetFileArray($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'][0]);
										$MPHBIG = CFile::ResizeImageGet($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'][0], array('width' => 295, 'height' => 197), 2);
									?>
									<div class="it2b">
										<a href="<?=$FILE1['SRC']?>" class="img1" data-fancybox="gal<?=$arItem['ID']?>">
											<img src="<?=$MPHBIG['src']?>" alt="">
											<? if(count($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'])>1){ ?>
												<span class="enumNewPhoto">+<?=count($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'])-1?> фото</span>
											<? } ?>
										</a>
										<? if(count($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'])>1){ ?>
											<div hidden style="display:none">
												<? foreach($arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'] as $kF=>$vF){
													$FILE1 = CFile::GetFileArray($vF);
													$MPHBIG = CFile::ResizeImageGet($vF, array('width' => 295, 'height' => 197), 2);
													?>
													<a href="<?=$FILE1['SRC']?>" data-fancybox="gal<?=$arItem['ID']?>">
														<img src="<?=$MPHBIG['src']?>" alt="">
													</a>
												<? } ?>
											</div>
								...
								<? if($arItem['DISPLAY_PROPERTIES']['VIDEO2']['VALUE']){ ?>
									<?
									//$FILE1 = CFile::GetFileArray($arItem['DISPLAY_PROPERTIES']['VIDEO2']['VALUE']);
									$MPHBIG = CFile::ResizeImageGet($arItem['DISPLAY_PROPERTIES']['VIDEO2']['VALUE'], array('width' => 295, 'height' => 197), 2);
									?>
								<div class="it2b">
									<a href="<?=$arItem['DISPLAY_PROPERTIES']['VIDEO2']['DESCRIPTION']?>" class="img1 vid" data-fancybox="gal_vid<?=$arItem['ID']?>">
										<img src="<?=$MPHBIG['src']?>" alt="">
									</a>
								</div>
								<? } ?>
		...
		<?endif;?>
					<div class="nltBtn"><a href="/catalog/">Перейти в каталог</a></div>
	</div></div>
<?endif;?>