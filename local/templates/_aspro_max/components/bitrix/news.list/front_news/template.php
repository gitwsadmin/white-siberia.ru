	...
	<?endif;?>
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				if($useImageAlternate){					
					...
					if($arParams['SIZE_IN_ROW'] == 3 || $arParams['SIZE_IN_ROW'] == 3){
				...
				<?if($arParams['HALF_BLOCK'] != 'Y' || ($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $indexItem < 2)):?>
					<div class="item-wrapper col-md-3 col-sm-6 col-xs-6 col-xxs-12 clearfix <?=$arItem['PROPERTIES']['TYPE_BLOCK']['VALUE_XML_ID'];?> <?=$dop_class;?><?=($bSlider ? ' item-width-261' : '');?>" data-ref="mixitup-target">
				<?endif;?>
					...
					<?endif;?>
						...
						<div class="inner-text<?=($bShowSection ? ' with-section' : '');?><?=($bActiveDate ? ' with-date' : '');?><?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-fon' : '');?>">
							...
							<div class="title<?=(($bBigBlock || $arParams['ALL_BLOCK_BG']=='N') && $arParams['TITLE_SHOW_FON'] != 'Y' ? ' font_mlg' : '' /*(!$bBordered || $arParams['SIZE_IN_ROW'] == 5 ? '' : ' font_md')*/);?>">
								...
								<div class="preview-text muted777 font_xs"><?
