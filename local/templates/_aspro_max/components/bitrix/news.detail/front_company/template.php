...
<div class="content_wrapper_block <?=$templateName;?>">
	<div class="maxwidth-theme <?=$arParams['TYPE_BLOCK'] == 'type2' ? '' : 'wide'?> mwnew">
		<?// preview image
		...
		<div class="item-views company lazy <?=$lightText ? 'company_light_text' : '';?> <?=$arParams['TYPE_IMG'];?> <?=$arParams['TYPE_BLOCK'];?><?=($arParams['TYPE_IMG'] == 'lg' ? ' ' : ' with-padding');?>" <?if($arResult['BG_IMG']):?>data-src="<?=$arResult['BG_IMG'];?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($arResult['BG_IMG']);?>)"<?endif;?>>
			<div class="company-block">
				<div class="top-block"><?if(in_array('NAME', $arParams['FIELD_CODE']) && $arResult['FIELDS']['NAME']):?>
											<h3><?=CMax::showIconSvg("forh1", SITE_TEMPLATE_PATH."/images/svg/forh1.svg");?><?=$arResult['FIELDS']['NAME']?></h3>
										<?endif;?></div>
				<div class="row flexbox<?=($arParams['TYPE_BLOCK'] == 'type2' ? ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? '' : ' flex-direction-row-reverse') : ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? ' flex-direction-row-reverse' : ''))?>">
					<div class="col-md-6<?#=($arParams['TYPE_BLOCK'] == 'type2' ? ($arParams['TYPE_IMG'] == 'md' ? 8 : 9) : 6);?> text-block col-xs-12">
						<div class="item">
							<div class="item-inner">
								<div class="text">
									<?ob_start();?>
										<?if($bShowUrl):?>
											<a class="show_all" href="<?=$arResult['DISPLAY_PROPERTIES']['URL']['VALUE'];?>">
										<?else:?>
									...
									<div class="js-tizers" style="margin-top:38px;"></div>
					...
					<div class="col-md-6<?#=($arParams['TYPE_BLOCK'] == 'type2' ? ($arParams['TYPE_IMG'] == 'md' ? 4 : 3) : 6);?> image-block col-xs-12">
