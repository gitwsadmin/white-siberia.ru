...
<?if($arElement && $arElement["PROPERTY_COLLECTION_TEMPLATE_ENUM_ID"]){
	//var_dump($arElement);
	$arDisplayRes = CIBlockPropertyEnum::GetList(Array("DEF"=>"DESC", "SORT"=>"ASC"), Array("ID"=>$arElement["PROPERTY_COLLECTION_TEMPLATE_ENUM_ID"]));
	if($arDisplay = $arDisplayRes->GetNext()){
		//var_dump($arDisplay);
        $arElement["DISPLAY_TEMPLATE"] = $arDisplay["XML_ID"];
    }
}
?>
