...
class CAsproYoutube extends CBitrixComponent
{
	...
	public function arResultSet(){
        ...
        $url = 'https://www.googleapis.com/youtube/v3/search?key=' . $this->access_token . '&channelId=' . $this->channel_id . '&part=snippet,id&order=date&maxResults=10';

        curl_setopt($ch, CURLOPT_URL, $url);

        $response    = curl_exec($ch);

        $arYoutubeVideos = json_decode($response, true);

        foreach($arYoutubeVideos['items'] as $key => $video):

						$arResult['ITEMS'][$key]['ID'] = $video['id']['videoId'];
						$arResult['ITEMS'][$key]['TITLE'] = $video['snippet']['title'];
            $arResult['ITEMS'][$key]['IMAGE'] = $video['snippet']['thumbnails']['medium']['url'];
