...
$arTypesConfig = array(
	'none' => array(
		...
		'YANDEX_PROP1_header_catalog' => 'header',
		'width',
		'height',
		'length',
		'weight',
		...
		'YANDEX_PROP1_header_sku' => 'header',
		'cus_skucolor',
		'size',
		'sku_picture',

		'YANDEX_PROP1_header_gtin' => 'header',
		'gtin',
		'okpd2',
		'tnved',
	),
...
<?
	...
	{
		$aTabs = array(
			array("DIV" => "yandex-settings-types", "TAB" => GetMessage('YANDEX_TAB1_TITLE'), "TITLE" => GetMessage('YANDEX_TAB1_DESC1')),
...
?>
		...
		<?echo BeginNote(), GetMessage('YANDEX_TYPE_NOTE'), EndNote();?>
			...
			<tr>
				<td colspan="2">
					<?= GetMessage('IPOL_ALI_EXPOR_DETAIL') ?>
				</td>
			</tr>


			<tr class="heading">
				<td colspan="2"><?=GetMessage('BX_CATALOG_EXPORT_YANDEX_COMMON_FIELDS1')?></td>
			</tr>

			<td>
				<td colspan="2">
					<?= GetMessage('BX_CATALOG_EXPORT_YANDEX_COMMON_FIELDS1_HELP') ?>
				</td>
			</td>
				...
				<? foreach ($arTypesConfig as $key => $arConfig): ?>
					<div id="config_<?=htmlspecialcharsbx($key)?>" style="padding: 10px; display: <? echo ($type == $key ? 'block' : 'none'); ?>;">
						<table width="90%" class="inner" style="text-align: center;">
							<? foreach ($arConfig as $headerText => $prop): ?>
								<? if ($prop == 'header') { ?>
									<tr class="heading">
										<td colspan="2"><?= GetMessage($headerText) ?></td>
									</tr>
								<? } else { ?>
									<tr>
										<td align="right"><?=GetMessage('YANDEX_PROP1_'.$prop) ?: GetMessage('YANDEX_PROP_'.$prop)?>: </td>
										<td style="white-space: nowrap;">
											<?php if ($prop == 'description') { ?>
												<select name="COMMON_FIELDS[DESCRIPTION]">
												...
												<small>(<?= $prop ?>)</small>
											<?php } else { ?>
												<?__yand_show_selector($key, $prop, $arIBlock, (isset($arTypeValues[$key][$prop]) ? $arTypeValues[$key][$prop] : ''))?>&nbsp;<small>(<?=htmlspecialcharsbx($prop)?>)</small>
											<?php } ?>
										</td>
									</tr>
								<? } ?>
		...
		<tr class="heading">
			<td colspan="2" valign="top"><?=GetMessage('YANDEX_PROPS_ADDITIONAL1')?></td>
		</tr>
		<tr>
			<td colspan="2" valign="top" style="text-align:center"><?=GetMessage('YANDEX_PROPS_ADDITIONAL1_HELP')?></td>
		</tr>
		<tr>
			<td colspan="2">
...
?>
			<tr class="heading">
				<td colspan="2"><? echo GetMessage('YANDEX_SKU_SETTINGS1');?></td>
			</tr>
			<tr>
