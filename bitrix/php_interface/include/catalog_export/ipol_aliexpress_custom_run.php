...
$arYandexFields = array(
	'name','description', 'picture', 'width', 'height',
	'length', 'weight', 'quantity', 'cus_skucolor',
	'size', 'sku_picture', 'gtin', 'okpd2', 'tnved', 'param'
);
...
{
function yandex_get_value($arOffer, $param, $PROPERTY, $arProperties, $arUserTypeFormat, $usedProtocol, $returnOnlyValue = false)
{
	...
	if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY]))
	{
		...
		elseif (!empty($value))
		{
			...
			if ($param_h == 'picture' || $param_h == 'sku_picture') {
				$pictures = array_map('yandex_text2xml', array_map('trim', explode(',', $value)));

				foreach ($pictures as $index => $picture) {
					$strProperty .= '<'. $param_h .'>'. $picture .'</'. $param_h .'>';
				}
			} else {
	...
	function yandexPrepareItems(array &$list, array $parents, array $options)
	{
		...
		foreach (array_keys($list) as $index)
		{
            ...
            $rs = CIBlockElement::GetList([], [
                'ID' => $row['ID'], 'IBLOCK_ID' => $row['IBLOCK_ID'],
            ]);
            ...
            if ($arProps['PICTURE_ALI']['VALUE']) {
                $row['DETAIL_PICTURE'] = $arProps['PICTURE_ALI']['VALUE'][0];
            }
            // --custom
			...
			if ($row[$descrField] !== null)
			{
                ...
                if ($arProps['DESCRIPTION_ALI']['VALUE']['TEXT']) {
                    $row['DESCRIPTION'] = yandex_text2xml(
                        $arProps['DESCRIPTION_ALI']['VALUE']['TYPE'] == 'HTML'
                            ? '<![CDATA['. $arProps['DESCRIPTION_ALI']['VALUE']['TEXT'] .']]>'
                            : TruncateText(
                            preg_replace_callback("'&[^;]*;'", 'yandex_replace_special', $arProps['DESCRIPTION_ALI']['VALUE']['TEXT']),
...
// custom
// AliExpress
$customAliExpressPictureID = 2008;
$customPicturesID = 880;
$yandexNeedPropertyIds = array($customAliExpressPictureID => 1);
// --custom
if ($fieldsExist)
{
...
$filterProducts  = (isset($FILTER_PRODUCTS)) ? $FILTER_PRODUCTS : '';

if ($filterProducts) {
	$filterProducts = preg_split('~[\.\,\; ]~', $filterProducts);
	$filterProducts = array_map('trim', $filterProducts);
	$filterProducts = array_map('intval', $filterProducts);
}

$filterAvailable = (isset($FILTER_AVAILABLE) && $FILTER_AVAILABLE == 'Y');
$filterQuantity  = (isset($FILTER_QUANTITY)) ? $FILTER_QUANTITY : 0;
...
if (empty($arRunErrors))
{
	...
	if ($filterProducts)
		$offersFilter['!ID'] = $filterProducts;
	if ($filterAvailable)
	...
	$event = new \Bitrix\Main\Event('ipol.aliexpress', "onExportFilterFill", array(
		'PRODUCTS_FILTER' => $filter, 
		'OFFERS_FILTER'   => $offersFilter,
	));
	...
	foreach($event->getResults() as $eventResult) {
		...
		if (isset($result['PRODUCTS_FILTER']) && is_array($result['PRODUCTS_FILTER'])) {
			$filter = $result['PRODUCTS_FILTER'];
		}

		if (isset($result['OFFERS_FILTER']) && is_array($result['OFFERS_FILTER'])) {
			$offersFilter = $result['OFFERS_FILTER'];
		}
	}
			...
			if ($needProperties || $needDiscountCache)
			{
				...
				else
				{
					foreach ($itemIdsList as $id)
					{
                        ...
                        // custom
                        // Change pictures
                        if ($items[$id]['PROPERTIES'][$customAliExpressPictureID]['VALUE']) {
                            if (count($items[$id]['PROPERTIES'][$customAliExpressPictureID]['VALUE']) > 1) {
                                unset($items[$id]['PROPERTIES'][$customAliExpressPictureID]['VALUE'][0]);
                                $items[$id]['PROPERTIES'][$customPicturesID]['VALUE'] = $items[$id]['PROPERTIES'][$customAliExpressPictureID]['VALUE'];
                            }
                        }
                        // custom
					}
		...
		if (!empty($items))
		{
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					{
						$offer['PARENT_ID'] = $row['ID'];

						$event = new \Bitrix\Main\Event('ipol.aliexpress', "onBeforeItemExport", array(
							'ITEM'  => $offer,
							'OFFER' => true,
						));
						...
						foreach($event->getResults() as $eventResult) {
							...
							$result = $eventResult->getParameters();
							$offer  = isset($result['ITEM']) ? $result['ITEM'] : array_merge($offer, $result);
						}

						
						$available = ' available="'.($offer['CATALOG_AVAILABLE'] == 'Y' ? 'true' : 'false').'"';
						$itemsContent .= '<offer id="'.$offer['ID'].'"'.$productFormat.$available .' group_id="'. $offer['PARENT_ID'] .'"' .">\n";
						$itemsContent .= '<barcode>'. $offer['ID'] .'</barcode>';
						...
						if (!empty($picture))
							$itemsContent .= "<picture>1".$picture."</picture>\n";
						unset($picture);
							...
							switch ($key)
							{
								...
								case 'quantity':
									...
									$defaultCatalogFields = [
										'width'    => ['CATALOG_WIDTH',  10],
										'height'   => ['CATALOG_HEIGHT', 10],
										'length'   => ['CATALOG_LENGTH', 10],
										'weight'   => ['CATALOG_WEIGHT', 1000],
										'quantity' => ['CATALOG_QUANTITY', 1],
									];

									if (empty($value) && strval($value) !== '0') {
										$value = $offer[ $defaultCatalogFields[$key][0] ];
									}

									$value = $value / $defaultCatalogFields[$key][1];

									if ($key == 'quantity' && ($filterQuantity > $value || $offer['CATALOG_AVAILABLE'] != 'Y')) {
										$value = 0;
									}

									$itemsContent .= '<'. $key .'>'. $value .'</'. $key .'>'. PHP_EOL;
								break;
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					// 	$result = $eventResult->getParameters();
					// 	$row = isset($result['ITEM']) ? $result['ITEM'] : array_merge($row, $result);
					// }

					$available = ' available="'.($row['CATALOG_AVAILABLE'] == 'Y' ? 'true' : 'false').'"';
                    $itemsContent .= '<offer id="'.$row['ID'].'"'.$productFormat.$available .' group_id="'. $row['ID'] .'">'."\n";
					$itemsContent .= '<barcode>'. $row['ID'] .'</barcode>';
					unset($available);
					...
					unset($referer);
   					...
   					!isset($row['RESULT_PRICE']['MIN_PRICE']) ?: $minPrice = $row['RESULT_PRICE']['MIN_PRICE'];
   					!isset($row['RESULT_PRICE']['FULL_PRICE']) ?: $fullPrice = $row['RESULT_PRICE']['FULL_PRICE'];
					...
					foreach ($arYandexFields as $key)
					{
						switch ($key)
						{
							...
							case 'quantity':
								...
								if (empty($value) && strval($value) !== '0') {
									$value = $row[ $defaultCatalogFields[$key][0] ];
								}

								$value = $value / $defaultCatalogFields[$key][1];

								if ($key == 'quantity' && ($filterQuantity > $value || $row['CATALOG_AVAILABLE'] != 'Y')) {
									$value = 0;
								}
