...
if(!class_exists("CAsproMax"))
{
	class CAsproMax{
		...
		public static function showRegionList(){
			...
			$iCalledID = ++$regionsList_call;?>			<?$frame = new \Bitrix\Main\Page\FrameHelper('allregions-list-block'.$iCalledID);?>
			<?$frame->begin();?>
				...
				<?ob_start();?>
					<?if(($bShowCounter && $arAddToBasketData["ACTION"] == "ADD") && $arAddToBasketData["CAN_BUY"]):?>
						...
						<div class="counter_block <?=$class;?>" data-item="<?=$id;?>">
							<span class="minus dark-color" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY_DOWN']; ?>" <?=($arAddToBasketData["SET_MIN_QUANTITY_BUY"] ? "data-min='".$arAddToBasketData["MIN_QUANTITY_BUY"]."'" : "")?>>-</span>
							<input type="text" class="text" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<?=$arAddToBasketData["MIN_QUANTITY_BUY"]?>" />
							<span class="plus dark-color" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['QUANTITY_UP']; ?>" <?=($arAddToBasketData["MAX_QUANTITY_BUY"] ? "data-max='".$arAddToBasketData["MAX_QUANTITY_BUY"]."'" : "")?>>+</span>
						</div>
						<?if($bWrap):?>
							</div>
						<?endif;?>
		...
		public static function showCalculateDeliveryBlock($productId, $arParams, $bSkipPreview = false){
			...
			<?if($productId > 0 && $arParams['CALCULATE_DELIVERY'] !== 'NOT'):?>
				...
				?>
				<?ob_start();?>
					<div class="calculate-delivery text-form  ncolor<?=($bWithPreview ? ' with_preview' : '')?>">
						...
						<div class="bottom-icons-panel swipeignore">
							<div class="bottom-icons-panel__content">
								<?foreach($arItems as $arItem) {?>
									...
									<a href="<?=$url?>" class="bottom-icons-panel__content-link<?=($bActive ? ' bottom-icons-panel__content-link--active' : ' dark-color');?> bottom-icons-panel-item_<?=$arProperty['XML_ID']?>" title="<?=$title;?>">
