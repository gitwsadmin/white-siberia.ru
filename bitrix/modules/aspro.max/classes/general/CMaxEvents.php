...
class CMaxEvents{
	...
	static function onAfterAjaxResponseHandler() {
		...
		$APPLICATION->arAdditionalChain = array_map(
			function($arElement) {
				$arElement['TITLE'] = self::replaceMarks($arElement['TITLE'], CMaxRegionality::$arSeoMarks);
				return $arElement;
			}
		, $APPLICATION->arAdditionalChain);

		$APPLICATION->sDocTitle = self::replaceMarks($APPLICATION->sDocTitle, CMaxRegionality::$arSeoMarks);
	}
	...
	static function OnEndBufferContentHandler(&$content)
	{
		...
		if(!defined('ADMIN_SECTION') && !defined('WIZARD_SITE_ID') && !defined('CUSTOM_CONTENT') || $bCompSaleOrderAjaxPost)
		{
			...
			// lazyload
			if( $GLOBALS['_USE_LAZY_LOAD_MAX_'] && !CMax::checkMask(Option::get('aspro.max', 'LAZY_LOAD_EXCEPTIONS', '')) ){
				...
				$bgPatterns = array(
					'/<('.$sTags.')([^>]*background\-image\:\s*url\s*\()/i'.BX_UTF_PCRE_MODIFIER,
					'/<('.$sTags.')([^>]*background\:.*?url\s*\([^>]*>)/i'.BX_UTF_PCRE_MODIFIER,
				);
			...
			if( $GLOBALS['_USE_GAPS_REPLACE_MAX_'] && (!defined('IGNORE_EOL_OPT') || IGNORE_EOL_OPT === false) ){
				$pattern = '/\n(\s*?)\n/is';
	...
	public static function onBeforeResultAddHandler($WEB_FORM_ID, &$arFields, &$arrVALUES){
		if(!defined('ADMIN_SECTION') && isset($_REQUEST['aspro_max_form_validate']))
		{
			...
			if($arTheme['HIDDEN_CAPTCHA'] == 'Y' && $arrVALUES['nspm'] && !isset($arrVALUES['captcha_sid']))
		  	...
		  	if($arTheme['SHOW_LICENCE'] == 'Y'  && ((!isset($arrVALUES['licenses_popup']) || !$arrVALUES['licenses_popup']) && (!isset($arrVALUES['licenses_popup_2AoK7']) || !$arrVALUES['licenses_popup_2AoK7']) && (!isset($arrVALUES['licenses_inline']) || !$arrVALUES['licenses_inline'])))
