...
class CMax{
		...
		if(is_array($arTheme) && $arTheme)
		{
			switch($type):
				...
				case 'search_title_component':
					...
					break;
				case 'mainpage':
					if($bLoadAjax && !self::checkAjaxRequest()):?>						<div class="content_wrapper_block"><div class="maxwidth-theme wide"></div></div>
					<?endif;?>
					<?if($bRestart && $subtype)
					{
						...
						<?if($iCountPhones > 1): // if more than one?>
							<ul class="dropdown <?=($bHaveIcons ? 'with_icons' : '')?>">
								...
								<?for($i = 0; $i < $iCountPhones; ++$i):?>
									...
									?>
									<li><a rel="nofollow" href="<?=$href?>" class="bold dark-color <?=(empty($description)?'no-decript':'decript')?>"><?=$icon?><?=$phone?><?=$description?></a></li>
								<?endfor;?>
								<?
	...
	public static function Start($siteID){
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			if($arTheme['SHOW_BASKET_NOTICE'] === 'Y'){
				\Aspro\Max\Functions\Extensions::init('notice');
			}
	...
	public static function ShowLogo(){
		...
		if($arImg = unserialize(Option::get(self::moduleID, "LOGO_IMAGE", serialize(array())), ['allowed_classes' => false]))
	...
	public static function ShowLogoFixed(){
		...
		$arTheme = self::GetFrontParametrsValues(SITE_ID);
		$text = '<a href="'.SITE_DIR.'">';
		if($arImg = unserialize(Option::get(self::moduleID, "LOGO_IMAGE_FIXED", serialize(array())), ['allowed_classes' => false]))
			$text .= '<img src="'.CFile::GetPath($arImg[0]).'" alt="'.$arSite["SITE_NAME"].'" title="'.$arSite["SITE_NAME"].'" data-src="" />';
		elseif($arImg = unserialize(Option::get(self::moduleID, "LOGO_IMAGE", serialize(array())), ['allowed_classes' => false]))
			$text .= '<img src="'.CFile::GetPath($arImg[0]).'" alt="'.$arSite["SITE_NAME"].'" title="'.$arSite["SITE_NAME"].'" data-src="" />';
		elseif(self::checkContentFile(SITE_DIR.'/include/logo_svg.php'))
			$text .= File::getFileContents($_SERVER['DOCUMENT_ROOT'].SITE_DIR.'/include/logo_svg.php');
		else
	...
	public static function GetQuantityArray(
		...
		if($arQuantityRights["SHOW_QUANTITY"]){
			...
			$totalHTMLs .= '<span title="'.$totalAmount.'" class="icon '.($totalCount > 0 ? 'stock' : ' order').'"></span><span class="value font_sxs">'.$totalText.'</span>';
			$totalHTMLs .='</div>';
		}
			...
			<div class="phone with_dropdown<?=($class ? ' '.$class : '')?>">
				<?if($bFooter):?>
					<div class="wrap">
						<div>
				<?endif;?>
					<?=static::showIconSvg("phone", SITE_TEMPLATE_PATH."/images/svg/".($bFooter ? "phone_footer" : "Phone_black").".svg");?><a rel="nofollow" href="<?=$href?>"><?=$phone?></a>
				<?if($bFooter):?>
						</div>
					</div>
				<?endif;?>
				<? if( $iCountPhones > 1 || $bHaveIcons || $bHaveDescription ): ?>
					...
					<?= static::showIconSvg("down", SITE_TEMPLATE_PATH."/images/svg/trianglearrow_down.svg"); ?>
				<? endif; ?>
			</div>
			<!-- /noindex -->
		<?endif;?>
	...
	public static function CopyFaviconToSiteDir($arValue, $siteID = ''){
		if(($siteID)){
			if(is_string($arValue) && $arValue) {
				$arValue = unserialize($arValue, ['allowed_classes' => false]);
			}
...
/*		if($arParams['TITLE_BLOCK_DETAIL_NAME'])
		...
		$html .= '<div class="option-font-bold">'.(strlen($arShop["URL"]) ? '<a class="dark_link" href="'.$arShop["URL"].'">' : '').$arShop["ADDRESS"].' '.$arShop['METRO_PLACEMARK_HTML'].(strlen($arShop["URL"]) ? '</a>' : '').'</div>';
		if($arShop['METRO'] || $arShop['SCHEDULE'] || $arShop['EMAIL'] || $arShop['PHONE'] || isset($arShop['QUANTITY'])){
			...
			if($arShop['PHONE']){
					...
					$html .= '<div class="property phone option-font-bold">'.$phone.'</div>';
				}

				$html .= (strlen($arShop['SCHEDULE']) ? '<div class="property schedule"><div class="value">'.$arShop['SCHEDULE'].'</div></div>' : '');

				$html .= (strlen($arShop['EMAIL']) ? '<div class="property email"><div class="value"><a class="dark_link" href="mailto:'.$arShop['EMAIL'].'">'.$arShop['EMAIL'].'</a></div></div>' : '');
			$html .= '</div>';
		}
		$html .= '</div>';
		return $html;
	...
	public static function CCatalogStore_GetList(
		...
		if( self::checkVersionModule('17.0.4', 'catalog') ){
		...
		if($arResultGroupBy["MULTI"] || $arResultGroupBy["GROUP"] || $arResultGroupBy["RESULT"]){
			$arRes = \CMaxCache::GroupArrayBy($arRes, $arResultGroupBy);
		}
