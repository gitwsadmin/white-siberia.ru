...
if($RIGHT >= "R"){
	...
	<?else:?>
		<div class="adm-info-message"><?=GetMessage("MAX_MODULE_DEVELOP_INFO");?></div>
		<br>
		<br>
		<?$tabControl->Begin();?>
		<a href="aspro.max_options_tabs.php" id="tabs_settings" target="_blank">
							...
							if($arOption['TYPE'] === 'array'){
											...
											<table class="aspro-admin-item-table">
												...
												<tr class="aspro-admin-item<?=(!$itemKey ? ' first' : '')?><?=($itemKey == $itemsKeysCount - 1 ? ' last' : '')?><?=($itemKey == $cnt ? ' new' : '')?>" data-itemkey="<?=$itemKey?>" style="text-align:center;"><?
														foreach($_arParameters as $_optionCode => $_arOption){
															$moduleClass::ShowAdminRow($_optionCode, $_arOption, $arTab, $arControllerOption, true);
														}
								...
								if($arOption["TYPE"] == 'note')
								{
									...
									<tr data-optioncode="<?=$optionCode;?>" <?=$style;?>>
										<?=$moduleClass::ShowAdminRow($optionCode, $arOption, $arTab, $arControllerOption, true);?>
									</tr>
