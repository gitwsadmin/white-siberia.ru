...
<?/* if($USER->IsAdmin()) print_r($arResult['ITEMS']);*/?>
<? global $arTheme;?>
<!--newtheme-->
<?if($arResult['ITEMS']):?>
	<?php
		$TOP = $arResult['ITEMS'][0];
		if(!$TOP['DETAIL_PICTURE']['SRC']) $TOP['DETAIL_PICTURE']['SRC'] = SITE_TEMPLATE_PATH.'/images/svg/noimage_content.svg';
		$SNAME = $arResult['SECTIONS'][$TOP['IBLOCK_SECTION_ID']]['NAME'];
	?>
<div class="content_wrapper_block front_news content_news2  without-border">
	<div class="maxwidth-theme only-on-front mwnew">
		<div class="top_block clearfix">
			<a class="blog_link" href="/blog/"><h3 class="pull-left"><i class="svg inline  svg-inline-forh1" aria-hidden="true"><svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.5" d="M15.9622 0H11.7494C10.8047 0 9.96248 0.596021 9.64888 1.48727L2.78015 21H6.99296C7.93768 21 8.77991 20.404 9.09352 19.5127L15.9622 0Z" fill="#F5333F"></path><path opacity="0.25" d="M15.54 0H8.96873C8.02401 0 7.18178 0.596021 6.86817 1.48727L0 21H8.14822L15.54 0Z" fill="#F5333F"></path><path d="M19.003 0H14.7902C13.8455 0 13.0033 0.596021 12.6896 1.48727L5.82092 21H10.0337C10.9785 21 11.8207 20.404 12.1343 19.5127L19.003 0Z" fill="#F5333F"></path></svg>
				</i>Блог</h3></a>
			<a href="/blog/" class="pull-right font_upper muted">Все статьи</a>
		</div>
		<div class="newBlogTempl">
			<div class="bigNewBlog">
				<a href="<?=$TOP['DETAIL_PAGE_URL']?>">
					<? if($SNAME){ ?>
						<span class="snameNewBlog"><?=$SNAME?></span>
					<? } ?>
					<span class="imgNewBlog lazy" style="background-image:url(/bitrix/templates/aspro_max/images/loaders/double_ring.svg)" data-src="<?=$TOP['DETAIL_PICTURE']['SRC']?>" data-bg="<?=$TOP['DETAIL_PICTURE']['SRC']?>">
						<img src="/bitrix/templates/aspro_max/images/loaders/double_ring.svg" data-src="<?=$TOP['DETAIL_PICTURE']['SRC']?>" class="lazy" alt="">
					</span>
					<span class="textNewBlog">
						<?=$TOP['NAME']?>
						<span class="viewNewBlog"><?=($TOP['ACTIVE_FROM']) ? $TOP['ACTIVE_FROM'] : date('d.m.Y', strtotime($TOP['TIMESTAMP_X']))?></span>
					</span>
				</a>
			</div>
			<? unset($arResult['ITEMS'][0]); ?>

			<div class="otListNewBLog">
				<? foreach ($arResult['ITEMS'] as $i => $arItem) {
				    $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem['IBLOCK_ID'], 'ELEMENT_EDIT'));
					$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem['IBLOCK_ID'], 'ELEMENT_DELETE'), array('CONFIRM' => Loc::getMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
					if(!$arItem['DETAIL_PICTURE']['SRC']) $arItem['PREVIEW_PICTURE']['SRC'] = SITE_TEMPLATE_PATH.'/images/svg/noimage_content.svg';
					?>
					<a href="<?=$arItem['DETAIL_PAGE_URL']?>">
						<span class="imgNewBlog lazy" style="background-image:url(/bitrix/templates/aspro_max/images/loaders/double_ring.svg)" data-src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" data-bg="<?=$arItem['PREVIEW_PICTURE']['SRC']?>">
							<img src="/bitrix/templates/aspro_max/images/loaders/double_ring.svg" data-src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" class="lazy" alt="">
						</span>
						<span class="textNewBlog">
							<?=$arItem['NAME']?>
						</span>
		...
		<div class="newBlogBtn">
			<span class="callback-block animate-load font_upper_xs colored" data-event="jqm" data-param-form_id="CALLBACK" data-name="callback">Получить консультацию</span>
		</div>
	</div>
</div>
...
<?if($arResult['ITEMS']):?>
	...
	<?if(!$arParams['IS_AJAX']):?>
		...
		<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL']):?>
			...
			<?else:?>
				<div class="top_block clearfix">
					<h3 class="pull-left"><?=CMax::showIconSvg("forh1", SITE_TEMPLATE_PATH."/images/svg/forh1.svg");?><?=$arParams['TITLE_BLOCK'];?></h3>
					<?if($arParams['SHOW_SUBSCRIBE'] == 'Y' && $arParams['TITLE_SUBSCRIBE']):?>
		...
		<div class="item-views news2 <?=$arParams['TYPE_IMG'];?><?=(!$bBordered ? '' : ' with-border');?><?=($arParams['HALF_BLOCK'] == 'Y' ? ' half-block' : '');?> <?=($bgSmallPlate ? 'small-bg-plate' : '');?> <?=$sTemplateMobile;?>">
			<div class="items<?=(!$arParams['INCLUDE_FILE'] ? '' : ' list');?> s_<?=$arParams['SIZE_IN_ROW'];?>">
				<div class="row flexbox <?=$sTemplateMobile;?><?=($bSlider ? ' swipeignore mobile-overflow mobile-margin-16 mobile-compact' : '');?><?=$bHasBottomPager ? ' has-bottom-nav' : ''?>" style="justify-content: center;">
	<?endif;?>
			<?
			$indexItem = 0;
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				// preview image
				$imageSrc = ($arItem['FIELDS']['DETAIL_PICTURE'] ? $arItem['FIELDS']['DETAIL_PICTURE']['SRC'] : '');
				...
				<?endif;?>
						...
						<?if($bImage):?>
							<div class="image shine<?=($bLineImg ? ' pull-left' : '');?>">
								<?if($bDetailLink):?>
									<a href="<?=$arItem['DETAIL_PAGE_URL']?>">
								<?endif;?>
									<span class="<?=(!$bBordered ? 'rounded3' : '');?> bg-fon-img lazy<?=$position;?>" data-src="<?=$imageSrc?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>)">
										<?#=$arItem['NAME'];?>
									</span>
								<?if($bDetailLink):?>
									</a>
					...
					<?endif;?>
						<div class="inner-text<?=($bShowSection ? ' with-section' : '');?><?=($bActiveDate ? ' with-date' : '');?><?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-fon' : '');?>">
							...
							<?endif;?>
							<div class="title<?=(($bBigBlock || $arParams['ALL_BLOCK_BG']=='N') && $arParams['TITLE_SHOW_FON'] != 'Y' ? ' font_mlg' : '' /*(!$bBordered || $arParams['SIZE_IN_ROW'] == 5 ? '' : ' font_md')*/);?>">
								<?if($bDetailLink):?>
									<a href="<?=$arItem['DETAIL_PAGE_URL']?>">
								<?endif;?>
								<?=$arItem['NAME'];?>
