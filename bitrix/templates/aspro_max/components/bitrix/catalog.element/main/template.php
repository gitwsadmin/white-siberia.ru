	...
	<div class="right_info">
		<div class="info_item hidden-md hidden-lg">
			...
			<?ob_start()?>
                ...
                <?$path = SITE_DIR."include/element_credit_info.php"?>

                <?$APPLICATION->IncludeFile($path, Array(), Array("MODE" => "html",  "NAME" => "Информация о кредитах"));?>
...
$component
            ...
            <?/*$this->SetViewTarget('PRODUCT_SIDE_INFO', 300); ?>
            <? $GLOBALS['arBuyServicesFilter']['ID'] = $arResult['SERVICES']; ?>
            <?$APPLICATION->IncludeComponent(
							...
							<div class="col-md-3 col-sm-4 col-xs-6">
								<div class="item">
									<div class="wrap"><a href="<?=$arPhoto['DETAIL']['SRC']?>" class="fancy" data-fancybox="small-gallery" target="_blank" title="<?=$arPhoto['TITLE']?>">
										<img data-src="<?=$arPhoto['THUMB']['src']?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arPhoto['THUMB']['src']);?>" class="lazy img-responsive inline" title="<?=$arPhoto['TITLE']?>" alt="<?=$arPhoto['ALT']?>" /></a>
									</div>
								</div>
							</div>
						<?endforeach;?>
